...
public class IgniteNodeUbuntu {
    private IgniteCache<Integer, String> firstTestcache;
    private Ignite ignite = Ignition.start("/usr/local/apache-ignite-2.4.0-src/examples/config/example-ignite.xml");

    public IgniteNodeUbuntu() {
        // 调用ignite方法向cache中创建键值对类型
        firstTestcache = ignite.getOrCreateCache("firstTestCache");
    }

    public static void main(String[] args) throws SQLException, ClassNotFoundException {
        IgniteNodeUbuntu igniteNodeUbuntu = new IgniteNodeUbuntu();
        ...
        while (true) {
            ...
            if (key.equals("88") && value.equals("88"))
                break;
            igniteNodeUbuntu.setKV(Integer.parseInt(key), value);
        }
        ...
        while (true) {
            ...
            if (getkey == 88)
                break;
            System.out.println(igniteNodeUbuntu.getKV(getkey));
        }
    }
